import { createChatChannelPlugin, buildChannelOutboundSessionRoute, DEFAULT_ACCOUNT_ID, } from "openclaw/plugin-sdk/core";
import { listClawTileAccountIds, resolveClawTileAccount, } from "./accounts.js";
...
const CLAWTILE_CHANNEL = "clawtile";
/**
...
const clawTileConfigAdapter = {
    listAccountIds: (cfg) => listClawTileAccountIds(cfg),
    resolveAccount: (cfg, accountId) => resolveClawTileAccount({ cfg, accountId }),
    isEnabled: (account) => account.enabled,
    isConfigured: (account) => Boolean(account.cloudUrl && account.apiKey),
    unconfiguredReason: (account) => {
        if (!account.cloudUrl)
            return "cloudUrl is not set in channels.clawtile";
        if (!account.apiKey)
            return "apiKey is not set in channels.clawtile";
        return "";
    },
};
// ---------------------------------------------------------------------------
...
// The channel plugin
// ---------------------------------------------------------------------------
export const clawTilePlugin = createChatChannelPlugin({
    base: {
        id: CLAWTILE_CHANNEL,
        meta: {
            id: CLAWTILE_CHANNEL,
        ...
        gateway: {
            startAccount: async (ctx) => {
                ...
                if (!account.cloudUrl || !account.apiKey) {
                    ctx.log?.error?.(`[${account.accountId}] ClawTile not configured (missing cloudUrl or apiKey)`);
                    throw new Error("ClawTile channel not configured");
                }
                ctx.log?.info(`[${account.accountId}] Connecting to ClawTile cloud: ${account.cloudUrl}`);
                ...
                if (!channelRuntime) {
                    ctx.log?.warn?.("[clawtile] channelRuntime not available -- AI dispatch will not work");
                }
                connector.setOnMessage(async (frame) => {
                            ...
                            await pluginRuntime.channel.reply.dispatchReplyWithBufferedBlockDispatcher({
                                ...
                                dispatcherOptions: {
                                    deliver: async (payload, _info) => {
                                        const replyText = typeof payload.text === "string"
                                            ? payload.text
                    ...
                    if (frame.type === "event" && frame.event === "device.online") {
                        ctx.log?.info?.(`[clawtile] Device ${frame.payload.deviceId} came online`);
                    }
                    if (frame.type === "event" && frame.event === "device.offline") {
                        ctx.log?.info?.(`[clawtile] Device ${frame.payload.deviceId} went offline`);
                    }
                });
                connector.setOnConnected(() => {
                    ...
                    try {
                        ...
                        for (const tool of todoTools) {
                            pluginRuntime.agent?.tools?.register?.(tool);
                        }
                ...
                function syncTodosToServer() {
                    try {
                        ...
                        req.on("error", (e) => ctx.log?.warn?.(`[clawtile] Todo sync failed: ${e.message}`));
                        req.write(body);
                        req.end();
                    }
                    catch (err) {
