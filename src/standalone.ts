...
const config: ClawTileChannelConfig = {
  cloudUrl:
    process.env.CLAWTILE_CLOUD_URL ?? "ws://localhost:3000/ws/openclaw",
...
async function main() {
  if (!config.apiKey) {
    console.warn(
      "[main] CLAWTILE_API_KEY not set -- running in demo mode with empty key"
    );
  }
  ...
  connector.setOnConnected(() =>
    console.log("Connected to ClawTile cloud platform")
  );
