...
function loadTodos(): TodoData {
  ...
  if (!fs.existsSync(TODO_FILE)) {
    const initial: TodoData = { todos: [], nextId: 1 };
...
// ─── Tool: Add Todo ─────────────────────────────────
export function createAddTodoTool(): AgentTool {
  return {
    label: "Add Todo",
    name: "clawtile_todo_add",
    description:
      "添加一条待办事项或备忘。当用户说'帮我记一下...'、'添加待办...'、'提醒我...'、'记录...'时使用此工具。",
    parameters: Type.Object({
      text: Type.String({ description: "待办事项内容" }),
      remindAt: Type.Optional(
        Type.String({
          description:
            "提醒时间，ISO 8601 格式，如 2026-04-07T15:00:00。如果用户没提到时间则不填。",
        })
      ),
    }),
    execute: async (_toolCallId: string, args: unknown) => {
      const { text, remindAt } = args as { text: string; remindAt?: string };
...
// ─── Tool: List Todos ─────────────────────────────────
export function createListTodosTool(): AgentTool {
  return {
    label: "List Todos",
    name: "clawtile_todo_list",
    description:
      "查看待办事项列表。当用户说'我有什么待办'、'看看任务'、'今天有什么事'、'待办列表'时使用此工具。",
    parameters: Type.Object({
      showDone: Type.Optional(
        Type.Boolean({ description: "是否显示已完成的待办，默认 false" })
      ),
    }),
    execute: async (_toolCallId: string, args: unknown) => {
      const { showDone } = args as { showDone?: boolean };
      const data = loadTodos();
      ...
      if (done.length > 0) result += `，${done.length} 项已完成`;
...
// ─── Tool: Complete Todo ─────────────────────────────
export function createCompleteTodoTool(): AgentTool {
  return {
    label: "Complete Todo",
    name: "clawtile_todo_complete",
    description:
      "将待办事项标记为已完成。当用户说'XX已经完成了'、'完成XX'、'XX做好了'时使用此工具。",
    parameters: Type.Object({
      keyword: Type.String({
...
// ─── Tool: Delete Todo ──────────────────────────────
export function createDeleteTodoTool(): AgentTool {
  return {
    label: "Delete Todo",
    name: "clawtile_todo_delete",
    description:
      "删除一条待办事项。当用户说'删掉XX'、'移除XX待办'、'不要XX了'时使用此工具。",
    parameters: Type.Object({
      keyword: Type.String({
        description: "待办事项的关键字，用于匹配待办内容",
      }),
...
// ─── Export all tools ─────────────────────────────
export function createAllTodoTools(): AgentTool[] {
  return [
    createAddTodoTool(),
    createListTodosTool(),
